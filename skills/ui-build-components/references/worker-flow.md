...
- **`replaces`** — 이 프리미티브가 감싸는 네이티브 HTML 태그명 (예: Button → `"button"`, Input → `"input"`). 화면 조립 검증(`ui-assemble-screens/scripts/eslint-raw-tag-gate.mjs`)이 "프리미티브 있는데 raw 태그 박음" 을 차단할 때 이 필드에서 금지 태그를 유도. **단일 네이티브 태그를 drop-in 대체하는 경우만** 채움 — Radix 합성(select / dialog / tabs 등)처럼 단일 태그 대체가 아니면 생략.
