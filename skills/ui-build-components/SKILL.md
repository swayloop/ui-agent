...
`design/DESIGN.md` 의 토큰 / variant / 원칙을 기준으로 shadcn 컴포넌트 + Storybook story + manifest 작성. 표현은 Tailwind 클래스로 (consumer 의 `.css` 의 `@theme` 가 자동 인식). 검증은 **prettier + eslint** (`eslint-plugin-better-tailwindcss` 의 `no-unknown-classes` + `no-restricted-classes`) 로 미정의 토큰 / arbitrary value 차단.
...
0. **사전 확인** — 현재 에이전트 (Claude / Codex 중 본인) 기준. 미충족 항목 있으면 사람 결정 대기. 상세 → `references/example.md` "0. 전제":
   ...
   - **`eslint-plugin-better-tailwindcss` 설치** — `package.json` 의 dep 및 `eslint.config.js` 에 등록 여부. 없으면 `pnpm add -D eslint-plugin-better-tailwindcss` + config 스니펫 안내 (step 6 lint 강제용). 이 플러그인은 cva / cn / clsx / twMerge / tv 등을 기본 callee 로 인식하며, **cva 의 nested `variants` 객체 안 클래스도 검사**한다 (`eslint-plugin-tailwind-v4` 는 base 문자열까지만)
...
6. 검증 (SKILL 절차 안):
   ...
   - `pnpm eslint <대상>` — 두 rule 병행:
     - `better-tailwindcss/no-unknown-classes` (+ `settings.better-tailwindcss.entryPoint`) → DESIGN.md 와 `@theme` 에 없는 토큰 차단 (`bg-VARIANTBAD` 등)
     - `better-tailwindcss/no-restricted-classes` (regex `\[(...)\](?!:)`) → arbitrary value 차단 (`bg-[#0066cc]` / `p-[13px]`)
...
- **DESIGN.md → 컴포넌트 변환 한 라운드 (Button) + eslint-plugin-better-tailwindcss 셋업** → `references/example.md`
